...
export default class AdPickerScreen extends React.Component {
  ...
  await   this.socket.on('adSetCreated', async(data)=>{
      ...
      _deleteAd= async (item) => {

      await  this.socket.emit('deleteAd', {clientUsername:await SecureStore.getItemAsync('clientSelectedUsername'),key:item.key});
      await   this.socket.on('adDeleted', async(data)=>{
      });

      var ads= this.state.dataSource;

      var index = ads.indexOf(item);
      ads.splice(index, 1);
      this.setState({dataSource:ads});
  ...
  _filterAd = async (item) => {
    ...
    render() {
...

        {
            return (
                ...
                <View style={styles.listText}>
                  ...
                  <View>
                  <Button title="Delete" onPress={()=> this._deleteAd(item)} />
      ...
      />
        ...
        <View>
        <Button title="Create New Ad" onPress={this._createAdSet} />
        </View>
