...
async componentDidMount(){
  ...
  _approveShootPlan= async () => {

  await this.socket.emit('approveShootPlan', {key:await SecureStore.getItemAsync('shootSelectedKey'),clientUsername:await SecureStore.getItemAsync('clientSelectedUsername'), entity:await SecureStore.getItemAsync('entityToken'), msg:'Approve Shoot Plan',description:this.state.description});
  ...
  action(){
    if (this.state.shootPlanApproved==false) {
      return(
        <View>
          <Button title="Approve Shoot Plan" onPress={this._approveShootPlan} />
          </View>
